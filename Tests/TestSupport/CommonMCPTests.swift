...
public struct CommonMCPTests {
    ...
    /// Assert helper that throws on failure (replaces #expect for non-test-target usage)
    func expect(_ condition: Bool, _ message: String = "Assertion failed") throws {
        guard condition else {
            throw TestError.invalidResponse(message)
        }
    }
    ...
    public func testInitialize() async throws {
        ...
        try expect(capabilities["tools"] != nil, "Should have tools capability")
        try expect(capabilities["resources"] != nil, "Should have resources capability")
        try expect(capabilities["prompts"] != nil, "Should have prompts capability")
    }
    ...
    public func testToolsList() async throws {
        ...
        try expect(tools.count == 1, "Should have exactly 1 tool")

        let addNumbersTool = tools[0]
        try expect(addNumbersTool["name"] as? String == "add_numbers")
        try expect(addNumbersTool["description"] as? String == "Adds two numbers together and returns the sum")
        ...
        try expect(inputSchema["type"] as? String == "object")
        ...
        try expect(properties["a"] != nil, "Should have 'a' property")
        try expect(properties["b"] != nil, "Should have 'b' property")

        guard let required = inputSchema["required"] as? [String] else {
            throw TestError.invalidResponse("No required array in inputSchema")
        }
        try expect(required.contains("a"), "Should require 'a'")
        try expect(required.contains("b"), "Should require 'b'")
    }
    ...
    public func testToolsCall() async throws {
        ...
        try expect(content.count == 1, "Should have 1 content item")

        let firstContent = content[0]
        try expect(firstContent["type"] as? String == "text")
        ...
        try expect(abs(sum - 8.7) < 0.001, "Sum should be 8.7")
    }
    ...
    public func testToolsCallWithIntegers() async throws {
        ...
        try expect(sum == 30.0, "Sum should be 30.0")
    }
    ...
    public func testToolsCallNonexistentTool() async throws {
        ...
        try expect(message.contains("not found") || message.contains("Tool"), "Error message should mention tool not found")
    }
    ...
    public func testResourcesList() async throws {
        ...
        try expect(resources.count == 1, "Should have exactly 1 resource")

        let piResource = resources[0]
        try expect(piResource["uri"] as? String == "math://constants/pi")
        try expect(piResource["name"] as? String == "pi_constant")
        try expect(piResource["mimeType"] as? String == "text/plain")

        let description = piResource["description"] as? String ?? ""
        try expect(description.contains("pi") || description.contains("π"), "Description should mention pi")
    }
    ...
    public func testResourcesRead() async throws {
        ...
        try expect(contents.count == 1, "Should have 1 content item")

        let piContent = contents[0]
        try expect(piContent["uri"] as? String == "math://constants/pi")
        try expect(piContent["mimeType"] as? String == "text/plain")
        try expect(piContent["text"] as? String == "3.14159")
    }
    ...
    public func testResourcesReadNonexistent() async throws {
        ...
        let message = error["message"] as? String ?? ""
        try expect(message.contains("not found") || message.contains("Resource"), "Error should mention resource not found")
    }
    ...
    public func testPromptsList() async throws {
        ...
        try expect(prompts.count == 1, "Should have exactly 1 prompt")

        let explainPrompt = prompts[0]
        try expect(explainPrompt["name"] as? String == "explain_math")

        let description = explainPrompt["description"] as? String ?? ""
        try expect(description.contains("explain") || description.contains("math"), "Description should mention explaining math")
        ...
        try expect(arguments.count == 1, "Should have 1 argument")

        let conceptArg = arguments[0]
        try expect(conceptArg["name"] as? String == "concept")
        try expect(conceptArg["required"] as? Bool == true)
    }
    ...
    public func testPromptsGet() async throws {
        ...
        try expect(messages.count == 1, "Should have 1 message")

        let message = messages[0]
        try expect(message["role"] as? String == "user")

        guard let content = message["content"] as? [String: Any] else {
            throw TestError.invalidResponse("No content in message")
        }

        try expect(content["type"] as? String == "text")

        let text = content["text"] as? String ?? ""
        try expect(text.contains("derivatives"), "Prompt should contain the concept")
        try expect(text.contains("explain"), "Prompt should ask to explain")
    }
    ...
    public func testPromptsGetMissingArgument() async throws {
        ...
        let message = error["message"] as? String ?? ""
        try expect(message.contains("concept") || message.contains("argument"), "Error should mention missing argument")
    }
}
